...
from evaluation.eval_helper import *
from evaluation.test_metrics import * #TODO: remove as all metrics will be included in metrics.py
...
class ACFLoss(Loss):
    ...
    def compute(self, x_fake):
        ...
        # print('', acf_fake-self.acf_real.to(x_fake.device))
...
class CrossCorrelLoss(Loss):
    def __init__(self, x_real, max_lag=64, **kwargs):
        ...
        print('cross_correl_real:', self.cross_correl_real)
        # print('cross_correl_real shape:', self.cross_correl_real.shape)
        self.max_lag = max_lag

    def compute(self, x_fake):
        cross_correl_fake = self.metric.measure(x_fake,lags=self.lags).mean(0)[0]
        print('cross_correl_fake:', cross_correl_fake)
        loss = self.norm_foo(
            cross_correl_fake - self.cross_correl_real.to(x_fake.device)).unsqueeze(0)
        print('cross_correl_loss:', cross_correl_fake - self.cross_correl_real.to(x_fake.device))
        return loss
...
class cross_correlation(Loss):
    def __init__(self, x_real, **kwargs):
        super(cross_correlation, self).__init__(**kwargs)
        self.x_real = x_real
