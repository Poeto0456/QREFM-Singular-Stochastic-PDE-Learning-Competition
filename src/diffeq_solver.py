...
class ControlledODE(torch.nn.Module):
    ...
    def prod(self, t, v, xi):
        ...
        if self.flag1d:
            xi_permuted = xi.permute(0, 2, 1) # [B, C, X]
        else:
            xi_permuted = xi.permute(0, 3, 1, 2) # [B, C, X, Y]
            

        H = self.spde_func(z, xi_permuted)
        
        # ----------------------------------------

        out_ft = torch.zeros(out_size, device=H.device, dtype=H.dtype)
        if self.flag1d:
            v = torch.fft.fftn(H, dim=[2]) 
            v = torch.fft.fftshift(v, dim=[2]) 
            v = torch.stack([v.real, v.imag], dim=1) 
            v = v.permute(0,1,3,2) 
            out_ft[:, :, freqs[0][0]:freqs[0][1] ]  = v[:, :, freqs[0][0]:freqs[0][1] ] 
        else:
            v = torch.fft.fftn(H, dim=[2,3]) 
            v = torch.fft.fftshift(v, dim=[2,3]) 
            v = torch.stack([v.real, v.imag], dim=1) 
