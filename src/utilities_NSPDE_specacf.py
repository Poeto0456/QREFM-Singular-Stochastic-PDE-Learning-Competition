...
try:
    from evaluation.loss import ACFLoss as ACFLossManual
except ImportError:
    ACFLossManual = None
...
class SpectralLoss(torch.nn.Module):
    def __init__(self, eps=1e-8, use_log=True):
        super().__init__()
        self.eps = eps
    ...
    def forward(self, pred, target):
        ...
        targ_f = torch.fft.rfft(target, dim=-1)
        pred_psd = (pred_f.real**2 + pred_f.imag**2) / T
        targ_psd = (targ_f.real**2 + targ_f.imag**2) / T

        if self.use_log:
            pred_spec = torch.log(pred_psd + self.eps)
            targ_spec = torch.log(targ_psd + self.eps)
        else:
            pred_spec = pred_psd
            targ_spec = targ_psd
        return self.criterion(pred_spec, targ_spec)

class ACFLossFFT(torch.nn.Module):
    def __init__(self, max_lag=50, normalize=True):
        super().__init__()
    ...
    def forward(self, pred, target):
        # Normalize dims -> [Batch, Features, Time]
        if pred.dim() == 4:
            B, X, Y, T = pred.shape
            pred = pred.view(B, X*Y, T)
            target = target.view(B, X*Y, T)
        elif pred.dim() == 3:
        ...
        # Zero Padding & FFT
        pred_pad = F.pad(pred, (0, T))
        target_pad = F.pad(target, (0, T))
        pred_f = torch.fft.rfft(pred_pad, dim=-1)
        targ_f = torch.fft.rfft(target_pad, dim=-1)
        
        # PSD & IFFT
        pred_psd = pred_f * torch.conj(pred_f)
        targ_psd = targ_f * torch.conj(targ_f)
        pred_acf = torch.fft.irfft(pred_psd, n=2*T, dim=-1)[..., :T]
        targ_acf = torch.fft.irfft(targ_psd, n=2*T, dim=-1)[..., :T]

        # Normalize
        if self.normalize:
            pred_var = pred_acf[..., 0:1] + 1e-8
            targ_var = targ_acf[..., 0:1] + 1e-8
            pred_acf = pred_acf / pred_var
            targ_acf = targ_acf / targ_var

        lag = min(self.max_lag, T)
        return self.criterion(pred_acf[..., :lag], targ_acf[..., :lag])

#===========================================================================
# Data Loaders
#===========================================================================
def dataloader_nspde_2d(u, xi=None, ntrain=1000, ntest=200, T=51, sub_t=1, sub_x=4, batch_size=20, dataset=None):
    if xi is None: print('There is no known forcing')
    if dataset=='sns': T, sub_t, sub_x = 51, 1, 4
...
def eval_nspde(model, test_dl, myloss, batch_size, device):
    ...
    with torch.no_grad():
        for u0_, xi_, u_ in test_dl:    
            ...
            if u_pred.dim() == 5: u_pred = u_pred.squeeze(1)
            if u_.dim() == 5: u_ = u_.squeeze(1)
...
def train_nspde(model, train_loader, test_loader, device, myloss, batch_size=20, epochs=5000,
    ...
    criterion_spec = SpectralLoss(use_log=True).to(device)
    criterion_acf_train = ACFLossFFT(max_lag=50, normalize=True).to(device)
    ...
    try:
        for ep in range(epochs):
            model.train()
            train_loss, train_spec_loss, train_acf_loss = 0., 0., 0.
            ...
            for u0_, xi_, u_ in train_loader:
                ...
                # Losses
                loss_l2 = myloss(u_pred[..., 1:].reshape(curr_bs, -1), u_[..., 1:].reshape(curr_bs, -1))
                loss_spec = criterion_spec(u_pred[..., 1:], u_[..., 1:])
                loss_acf = criterion_acf_train(u_pred[..., 1:], u_[..., 1:])

                loss = loss_l2 + (spec_weight * loss_spec) + (acf_weight * loss_acf)

                train_loss += loss_l2.item()
                train_spec_loss += loss_spec.item()
                train_acf_loss += loss_acf.item()
            ...
            with torch.no_grad():
                for u0_, xi_, u_ in test_loader:
                    ...
                    # Manual ACF Monitor
                    if ACFLossManual is not None:
                        try:
                            T_dim = u_.shape[-1] - 1
                            pred_reshaped = u_pred[..., 1:].reshape(curr_bs, -1, T_dim).permute(0, 2, 1)
                            target_reshaped = u_[..., 1:].reshape(curr_bs, -1, T_dim).permute(0, 2, 1)
                            
                            acf_metric = ACFLossManual(x_real=target_reshaped, max_lag=50, stationary=True, name='val_acf')
                            val_acf_manual_total += acf_metric.compute(pred_reshaped).mean().item()
                        except: pass

            avg_train_l2 = train_loss / len(train_loader)
            avg_val_l2 = test_loss / len(test_loader)
            avg_val_acf = val_acf_manual_total / len(test_loader)

            if plateau_patience is None: scheduler.step()
            else: scheduler.step(avg_val_l2)
            
            if plateau_terminate is not None:
                early_stopping(avg_val_l2, model)
                if early_stopping.early_stop:
                    print("Early stopping")
                    break

            if ep % print_every == 0:
                losses_train.append(avg_train_l2)
                losses_test.append(avg_val_l2)
                print(f'Epoch {ep:04d} | Train L2: {avg_train_l2:.4f} | Train Spec: {train_spec_loss/len(train_loader):.4f} | Val L2: {avg_val_l2:.4f} | Val ACF: {avg_val_acf:.4f}')

        if time_train and time_eval: return model, losses_train, losses_test, times_train, times_eval 
        elif time_train: return model, losses_train, losses_test, times_train
        elif time_eval: return model, losses_train, losses_test, times_eval 
        else: return model, losses_train, losses_test
